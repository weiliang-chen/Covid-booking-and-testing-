...
public class PinCodeGeneration extends JFrame {
    private JButton continueButton;
    private JLabel bookingPin;
    private JPanel mainPanel;
    private JLabel bookingID;

    public PinCodeGeneration(String title, Application controller) {
        ...
        if(user != null)
        {
            ...
            if(serviceFacade.createBooking(user.getId(),facilities.get(0).getId(),"a covid test", "PCR", false, false))
            {
                Booking booking = null;
                ArrayList<Booking> currentBooking = serviceFacade.getAllBookingList();
                if(currentBooking.size() > 0)
                {
                    booking = currentBooking.get(0);
                }

                bookingID.setText(booking.getBookingId());
                bookingPin.setText(booking.getQrCode());
            }
        }
